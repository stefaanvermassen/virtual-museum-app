...
//[ExecuteInEditMode]
public class MuseumDiveSensor : MonoBehaviour {
	...
	void Start () {
		...
		if (err==1) {
			show_gyro_error_message=true;
			errormessage="ERROR: Dive needs a Gyroscope and your phone has none, we are trying to go to Accelerometer compatibility mode. Don't expect too much.";
		}
#elif UNITY_IPHONE
		initialize_sensors();
#endif
	...
	public void Rotate(float xAxis, float yAxis, FirstPersonController control) {
		...
		if (emulateMouseInEditor){
			if (axes == RotationAxes.MouseXAndY) {
				control.RotateVertical(-yAxis * sensitivityY);
				control.RotateHorizontal (xAxis * sensitivityX);
			} else if (axes == RotationAxes.MouseX) {
				control.RotateHorizontal (xAxis * sensitivityX);
			} else control.RotateVertical(-yAxis * sensitivityY);
		}
#elif UNITY_ANDROID
		...
		if (is_tablet==1) {
			control.SetRotation(rot*Quaternion.AngleAxis(90,Vector3.forward));
		} else control.SetRotation(rot);
#elif UNITY_IPHONE
		DiveUpdateGyroData();
		get_q(ref q0,ref q1,ref q2,ref q3);
		rot.x=-q2;
